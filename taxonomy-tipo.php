...
<section id="downloads-pagina">
	<div class="content">
		<header class="header-padrao">
			...
			<h2>Downloads por Tipo de Arquivo</h2>
		</header>
		<div class="col1">
					...
					<select id="area-de-interesse" name="area">
						...
						if ( $areas ) {
							foreach ( $areas as $area ) {
								echo '<option value="' . $area->slug . '">' . $area->name . '</option>' . "\n";
							}
						}
						?>											
					</select>
					...
					<select id="tipo-de-arquivo" name="tipo">
						...
						if ( $tipos ) {
							foreach ( $tipos as $tipo ) {
								$selected = $term->slug == $tipo->slug ? " selected='selected'" : "";	
...
//						'posts_per_page' => 1, //mudar quando for online
					...
					$tipos_loop = new WP_Query( $args );

					$userRole = $current_user->roles;
					if ( $tipos_loop ) :
						while ( $tipos_loop->have_posts() ) : $tipos_loop->the_post();
							?>
							<tr>
								<?php
